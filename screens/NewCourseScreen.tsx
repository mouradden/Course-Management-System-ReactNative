...
const NewCourseScreen = () => {
    ...
    // console.log(title, description, instructor, schedule);
    const newCourse = { title : title, description: description, instructor: instructor, schedule: schedule};
    console.log(newCourse);
    const addCourse = () => {
        axios.post('http://localhost:3000/course/addNew', newCourse)
        .then((respose)=>{
            console.log('A course added successfully ', respose.data);
            navigation.goBack();
        })
        .catch((error)=>{
  ...
  return (
    <View className='flex p-4 items-center'>
      ...
      <View className='flex flex-col items-center pl-2 gap-4'>
        ...
        <View className='border rounded w-24 bg-blue-400'>
            <Button
                onPress={addCourse}
                title="Add"
                color='white'
